...
export const getPost = async (req, res) => {
  try {
    ...
    if (speciesActual) query.speciesActual = { $regex: `^${speciesActual}$`, $options: "i" };
    if (speciesCommon) query.speciesCommon = { $regex: `^${speciesCommon}$`, $options: "i" };
  ...
  try {
    ...
    const populated = await Promise.all(
      comments.map(async (comment) => {
        const author = await User.findById(comment.authorId, { username: 1, profilePicUrl: 1 });
