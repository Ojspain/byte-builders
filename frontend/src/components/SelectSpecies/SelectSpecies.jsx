...
function SelectSpecies({
  ...
  const handleInputChange = (e) => {
    ...
    setSpeciesQuery(value); // Keep updating the UI text field

    // Extract the actual name by splitting at the " ("
    const extractedActualName = value.split(" (")[0].trim();

    // Match against the extracted actual name OR the raw typed value
    const matchedSpecies = speciesCandidates.find(
      (s) =>
        s.speciesActual === extractedActualName ||
        s.speciesActual === value.trim(),
    );

    // Set the object if found, otherwise null (using optional chaining)
    if (matchedSpecies) {
      setSelectedSpecies?.(matchedSpecies);
    } else {
